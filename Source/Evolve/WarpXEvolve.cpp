...
WarpX::Evolve (int numsteps)
{
    ...
    for (int step = istep[0]; step < numsteps_max && cur_time < stop_time; ++step)
    {
             ...
             electromagnetic_solver_id == ElectromagneticSolverAlgo::HybridPIC )
        {
            ...
            PushParticlesandDeposit(cur_time, skip_deposition, step);
        }
...
void
WarpX::PushParticlesandDeposit (amrex::Real cur_time, bool skip_current, int step)
{
    // Evolve particles to p^{n+1/2} and x^{n+1}
    // Deposit current, j^{n+1/2}
    for (int lev = 0; lev <= finest_level; ++lev) {
        PushParticlesandDeposit(lev, cur_time, DtType::Full, skip_current, step);
    }
}
...
WarpX::PushParticlesandDeposit (int lev, amrex::Real cur_time, DtType a_dt_type, bool skip_current, int step)
{
    ...
    mypc->Evolve(lev,
                 ...
                 *current_x, *current_y, *current_z,
                 ...
                 cur_time, dt[lev], a_dt_type, skip_current, step);
    if (! skip_current) {
#ifdef WARPX_DIM_RZ
